...
void set_field_value(RESENTMENT_package *pck, char *name, char *value){
    if (!strcmp(name, "name")) {
...
RESENTMENT_package *parse_package(char *filename){
    ...
    RESENTMENT_package *pck = malloc(sizeof(RESENTMENT_package));
    ...
    memset(pck, 0, sizeof(RESENTMENT_package));
...
RESENTMENT_package *package_load(char *root, char *name){
    ...
    RESENTMENT_package *pck = parse_package(full_path);
